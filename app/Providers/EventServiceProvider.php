<?php

namespace App\Providers;

use Illuminate\Support\Facades\Event;
use Illuminate\Foundation\Support\Providers\EventServiceProvider as ServiceProvider;

class EventServiceProvider extends ServiceProvider
{
    /**
     * The event listener mappings for the application.
     *
     * @var array
     */
    protected $listen = [
        'App\Events\SendRegistrationMail' => [
            'App\Listeners\RegistrationMailListeners'
        ],
        'App\Events\SendForgotPasswordMail' => [
            'App\Listeners\ForgotPasswordMailListeners'
        ],
        'App\Events\SendEmailVerificationEmail' => [
            'App\Listeners\EmailVerificationListeners'
        ],
        'App\Events\UpdateUserState' => [
            'App\Listeners\UpdateUserStateListener'
        ],
        'App\Events\SendMMGBookingMailToUser' => [
            'App\Listeners\SendMMGBookingMailToUserListener'
        ],
        'App\Events\SendMMGBookingMailToAdmin' => [
            'App\Listeners\SendMMGBookingMailToAdminListener'
        ],
    ];

    /**
     * Register any events for your application.
     *
     * @return void
     */
    public function boot()
    {
        parent::boot();

        //
    }
}
